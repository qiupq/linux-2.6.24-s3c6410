 ...
 *
 * $Id: s3c-ts.c,v 1.14 2008/09/24 01:53:32 dark0351 Exp $
 *
...
static int __init s3c_ts_probe(struct platform_device *pdev)
{
	...
	/* Get irqs */
	if (request_irq(IRQ_ADC, stylus_action, IRQF_SAMPLE_RANDOM, "s3c_action", ts->dev)) {
		printk(KERN_ERR "s3c_ts.c: Could not allocate ts IRQ_ADC !\n");
		iounmap(base_addr);
		return -EIO;
	}

	if (request_irq(IRQ_TC, stylus_updown, IRQF_SAMPLE_RANDOM, "s3c_updown", ts->dev)) {
