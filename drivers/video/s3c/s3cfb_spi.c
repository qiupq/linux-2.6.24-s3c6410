 ...
 *
 * $Id: s3cfb_spi.c,v 1.4 2008/09/09 01:39:49 eyryu Exp $
 *
...
{
	s3c_gpio_setpin(S3C_FB_SPI_CLK(ch), value);
}

inline void s3cfb_spi_lcd_dseri(int ch, int value)
{
	s3c_gpio_setpin(S3C_FB_SPI_MOSI(ch), value);
}

inline void s3cfb_spi_lcd_den(int ch, int value)
{
	s3c_gpio_setpin(S3C_FB_SPI_CS(ch), value);
}

inline void s3cfb_spi_set_lcd_data(int ch)
{
	s3c_gpio_cfgpin(S3C_FB_SPI_CLK(ch), 1);
	s3c_gpio_cfgpin(S3C_FB_SPI_MOSI(ch), 1);
	s3c_gpio_cfgpin(S3C_FB_SPI_CS(ch), 1);

	s3c_gpio_pullup(S3C_FB_SPI_CLK(ch), 0);
	s3c_gpio_pullup(S3C_FB_SPI_MOSI(ch), 0);
	s3c_gpio_pullup(S3C_FB_SPI_CS(ch), 0);
}
...
inline void s3cfb_spi_lcd_dclk(int ch, int value)
{
	s5p_gpio_setpin(S5P_FB_SPI_CLK(ch), value);
}

inline void s3cfb_spi_lcd_dseri(int ch, int value)
{
	s5p_gpio_setpin(S5P_FB_SPI_MOSI(ch), value);
}

inline void s3cfb_spi_lcd_den(int ch, int value)
{
	s5p_gpio_setpin(S5P_FB_SPI_nSS(ch), value);
}

inline void s3cfb_spi_set_lcd_data(int ch)
{
	s5p_gpio_cfgpin(S5P_FB_SPI_CLK(ch), 1);
	s5p_gpio_cfgpin(S5P_FB_SPI_MOSI(ch), 1);
	s5p_gpio_cfgpin(S5P_FB_SPI_nSS(ch), 1);

	s5p_gpio_pullup(S5P_FB_SPI_CLK(ch), S5P_GPIO_PUD_DISABLE);
	s5p_gpio_pullup(S5P_FB_SPI_MOSI(ch), S5P_GPIO_PUD_DISABLE);
	s5p_gpio_pullup(S5P_FB_SPI_nSS(ch), S5P_GPIO_PUD_DISABLE);
}
