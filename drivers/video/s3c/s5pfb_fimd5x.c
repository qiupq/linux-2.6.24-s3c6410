...
int s3cfb_init_registers(s3c_fb_info_t *fbi)
{
	...
	if (win_num == 0) {
		...
		s3c_fimd.vidcon0 |= S3C_VIDCON0_CLKVAL_F((int) ((clk_get_rate(lcd_clock) / s3c_fimd.lcd_pixclock) - 1));
...
static int s3cfb_set_alpha_level(s3c_fb_info_t *fbi, unsigned int level)
{
	...
	if (win_num == 0) {
		printk("WIN0 does not support alpha blending.\n");
		return -1;
	}
...
void s3cfb_set_gpio(void)
{
	...
	/* module reset */
	s5p_gpio_cfgpin(S5P_GPH3_6, S5P_GPH3_6_OUT);

	s5p_gpio_setpin(S5P_GPH3_6, 1);
	mdelay(100);

	s5p_gpio_setpin(S5P_GPH3_6, 0);
	mdelay(10);
